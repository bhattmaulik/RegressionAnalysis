# Getting Started with R for Econometrics {#sec-getting-started}
...
```{r}
#| echo: false
total_pkgs <- nrow(available.packages(repos = "https://cran.rstudio.com"))
```
...
mtcars |> select(am:carb) |> head() |> knitr::kable()
```
...
mtcars |> dplyr::filter(cyl==6) |> head() |> knitr::kable()
```
...
mtcars |> arrange(mpg)|> select(mpg:disp) |> head() |> knitr::kable()
```
...
mtcars |> arrange(desc(mpg)) |> select(mpg:disp) |> head() |> knitr::kable()
```
...
mtcars |> mutate(FuelEff = 1/mpg) |> select(am:FuelEff) |> head() |> knitr::kable()
```
...
The `group_by` argument changes the unit of analysis. For example, if we write the code `mtcars |> summarize(mean_mpg = mean(mpg))`, then the unit of analysis is all the the observations of the `mpg` column. But when we introduce `group_by`, by writing `mtcars |> group_by(cyl) |> summarise("Average MPG" = mean(mpg))`, then the unit of analysis is each level of `cyl` variable. Now the average of `mpg` is calculated for each level of `cyl`, instead of calculating it for the whole lot of observation.
