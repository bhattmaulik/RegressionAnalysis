...
```{r}
mod1 <- lm(Y~X2+X3+X4, data = Table7.6)
```

We can see that multiple regression also requires the `lm` function, just like classical linear regression model. To add more variables in the regression model, we have to use the `+` sign on the right hand side of the `~` sign.

We can see the model parameters and model diagnostics using the `easystats` set of packages.

```{r}
#| label: tbl-lm-params
#| tbl-cap: "Multiple Regression Parameters"
model_parameters(mod1)|> 
  display(format = "markdown", caption = "Multiple Regression Parameters")
```
